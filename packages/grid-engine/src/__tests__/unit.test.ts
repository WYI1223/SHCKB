...
describe('inferDropIntent', () => {
  test('place in empty area', () => {
    const r = inferDropIntent(createEmptyState(12), 0, 0, { colSpan: 12, rowSpan: 1 });
  ...
  test('reject on occupied cell', () => {
    ...
    if (!r1.ok) throw new Error();
    const r = inferDropIntent(r1.state, 2, 0, { colSpan: 12, rowSpan: 1 });
  ...
  test('hole-fill shrink-to-fit', () => {
    ...
    if (!r2.ok) throw new Error();
    s = r2.state;
    // proposed 6×4; cursor at col=7 row=4; only 5 cols (12-7) wide
    const intent = inferDropIntent(s, 7, 4, { colSpan: 6, rowSpan: 4 });
...
describe('validateState', () => {
  ...
  test('floating block is invalid under gravity, valid with gravity off', () => {
    const state = {
      blocks: [
        { id: 'a', col: 0, row: 5, colSpan: 2, rowSpan: 1, kind: 'markdown' },
      ],
      totalCols: 12,
    };
    expect(validateState(state).ok).toBe(false);
    expect(
      validateState(state).errors.some((e) => e.includes('not gravity-stable')),
    ).toBe(true);
    expect(validateState(state, { gravity: false }).ok).toBe(true);
  });
});
