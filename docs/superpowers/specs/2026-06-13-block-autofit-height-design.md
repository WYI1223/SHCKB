...
- 离屏盒再给一个**确定的高**（`MEASURE_CELL_PX`，足够大）：块在真实画布上是**填满
  画布给的格子**的，部分 Frame 把内容画在**绝对定位 / inset 的内层**（如 stationery
  纸片 slip 的 `.skb-paper`），无确定高时该层**塌成 0**——见下 E2E finding；
- 在 Frame 内挂**两层**子节点量：**AREA**（`height:100%`，撑满 Frame 内容盒 → 其
  `offsetHeight` 即该测量格高下的可用内容区）+ **CONTENT**（`height:auto`，RenderView
  的自然高）。`chrome = MEASURE_CELL_PX − area`；`fit = ceil((content + chrome +
  2*pad) / SLOT)`（[measureFit.fitFromContent](../../../apps/web/src/grid/measureFit.ts)）。
  **不需逐 Frame 知识、不读 getComputedStyle、不算宽度**，因为换行宽与 chrome 由构造正确。
  - 其中 `2*pad` 是画布对每块的**格子内缩**：提交后块活在 `rowSpan*slot − 2*pad` 高的
    格子里，故反推 rowSpan 必须把这段加回去（否则边界内容差最后 `2*pad`px 被裁）。

> **E2E finding（2026-06-14，MVP-9.5 跑通 e2e 时发现并修正）**：初版"量 Frame
> wrapper 外层 `offsetHeight`、`fit = ceil(outer/slot)`、不给确定高"在 **in-flow Frame**
> （graph-paper 默认壳、galley）下正确，但在 **stationery** 这类把内容画进绝对定位
> inset 内层的 Frame 上**外层塌成 0 → fit 恒为 1 → 内容被大幅裁切**（galley 6 行、
> stationery 却 1 行装 328px 内容）。且原式漏了 `2*pad` 格子内缩，边界内容会差几像素溢出。
> 修正＝上面的「确定高 + AREA/CONTENT 双层 + chrome = 格高 − area + 2*pad」，对**所有
> 填格型 Frame**（当前全部主题都是）构造正确。穿真 Frame、真实几何宽这两条红队结论不变。

测量点：
- **active 块**（实现里测量只在此发生）：编辑面是**纯 textarea**（源码，高度与渲染
  无关）。同时挂一个**离屏的 [MeasureProbe](../../../apps/web/src/grid/MeasureProbe.tsx)**
  （穿真 Frame + 上面的双层量法）经 `ResizeObserver` 观测 CONTENT 节点取 fit；gesture
  按 `max(floor, fit)` reconcile。owner 拍板另把**可见预览**做成编辑面里**右侧对齐的悬浮
  ghost**（见 §7，同一条 RenderView 路径）。source 长于渲染时 textarea 在块内可临时滚动
  （仅 active 态，无滚轮承诺针对静止/阅读态）。
- **inactive 块**：渲染上次 active gesture 提交的 `rowSpan`（不再测量）；内容/壳/主题
  变化的自愈在该块下次被激活时触发（§5.1）。
