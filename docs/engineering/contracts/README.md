...
**SoT 物理位置**：每个包的 `CONTRACT.md` 住在 `packages/<pkg>/CONTRACT.md`（贴源码同包根）。本索引页**只列已存在契约 + 一句话定位 + 链接**；不放契约本体内容。
...
（其他 package 契约随 ADR-0004 / 0014 落地补；Phase F carryover 提升时一并补：`block-foundation` / `plugin-markdown` / 等）
...
- **演化规则**：
  - CONTRACT surface 小步演化（minor / patch / breaking-but-not-induction）走 PR review，**不**每次都 supersede ADR
...
详 ADR 写作 method：[`process/methods/adr-discipline.md`](../../process/methods/adr-discipline.md) 的 foundational-ADR + contract-doc 分工节。
