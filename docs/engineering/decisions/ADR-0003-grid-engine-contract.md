...
### Induction 2 — 该层是依赖图叶子（leaf node）

Grid-engine 在 package import 关系图上：**只有别人指向它的箭头，它自己不指向任何 internal package**。源码层级 imports 只允许 TypeScript 标准库 + 类型工具。

→ 推出三条对外承诺，**这三条不是并列 invariant，是从"叶子位置"自然推出的副作用**：

- **Pure function** —— 不知道 IO 世界，做不出 IO
- **无内部 state** —— 不知道 storage 世界，藏不了 state；caller 自管 GridState，传入传出
- **完全 framework-agnostic** —— 不 import React / DOM / Drizzle / Lexical / Hono / plugin code / theme code

### Induction 3 — 只承载 block-as-AABB + layout invariant

Block 在 engine 视野下是 **不透明 AABB 矩形**，仅有 `{ id, kind, col, row, colSpan, rowSpan }` 6 字段进入 engine 关注范围。Kind 作 string tag 用于 intent inference / defaults lookup，但 engine **不解释 kind 语义**；block 的内容（`content_inline` / `props_json`）**完全不在本层**。

→ 推出：engine 是 **kind-opaque + content-opaque**。Plugin churn 不波及 engine；editor SoT（Lexical / textarea / canvas / 其他）也不波及 engine。
...
| Per-kind defaultSize 数值 | ADR-0014 `BlockPlugin.defaultSize` | Engine `inferDropIntent` 由 caller 传入 default 做 hole-fill clamp |
...
| Constrained-canvas **产品心智**（为什么不是 Notion / free canvas / Tetris） | frozen DI grid-redesign §1-2 + `product/prd/project.md` + `mental-model.md`（Phase C 待写） | 本 ADR 编码 invariant，不重决策心智 |
...
- 固定 12 col（来自 induction 1+3 不变量）—— 不响应式自适配；mitigate by viewport-level 12/6/1 切换（CSS-only，render layer）
...
- **Mental model**：`engineering/design/living/mental-model.md`（Phase C 待写）
- **Source DI docs**：
