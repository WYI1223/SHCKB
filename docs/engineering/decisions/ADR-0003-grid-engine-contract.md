...
- ✅ **允许 import**：TypeScript 标准库 / 类型工具 / 纯 utility lib（如 `zod` runtime 验证；不引入 framework / runtime concern）
- ❌ **不允许 import**：本 monorepo 任何其他 `@skb/*` package / React / DOM / Drizzle / Lexical / Hono / fetch / IndexedDB / 任何 framework 或 runtime API
