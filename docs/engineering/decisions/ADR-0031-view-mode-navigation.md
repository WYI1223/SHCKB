...
> **2026-06-18 收尾增补②（登录态「以访客预览」+ §11.1 终裁）**：本 ADR 的 read = 整库公开外链，但登录态此前无入口。收尾补上：侧栏目录改成**按 surface 取**——`read`/`note` 面即使登录也走 **public-tree 投影**（已发布页 + `/read` 链接）。**spec §11.1 由此终裁 = 登录作者进公开面看「访客真正的公开目录」，非作者全鞋柜。** 入口/出口住在新的**底部悬浮账户区**（「以访客预览」/「退出预览 → /edit」，与 sign out 并排）；公开面隐藏 edit/preview 切换。落地见 spec §13；WHAT 见 notepage-view PRD pass 7「Author can preview the public experience」。
...
- 2026-06-18 **收尾增补②**：§3 加「登录态以访客预览 + §11.1 终裁」注——侧栏目录按 surface 取（read/note 即使登录也走 public 投影），§11.1 终裁为登录作者进公开面看访客公开目录；配底部悬浮账户区的「以访客预览 / 退出预览」入口、公开面隐藏 edit/preview 切换。配 spec §13 + notepage-view PRD pass 7。
