...
test('published follow block clips (overflow:hidden); fix block scrolls (auto)', async ({ page }) => {
  ...
  const { slug } = await createMarkdownPage(page.request, {
    ...
    themeId: 'graph-paper',
    blocks: [
      { id: 'A', kind: 'markdown', col: 0, row: 0, colSpan: 6, rowSpan: 2, autofit: 'follow', content: md('## clipped\n\nbody') },
      { id: 'B', kind: 'markdown', col: 6, row: 0, colSpan: 6, rowSpan: 2, autofit: 'fix', content: md('## scrolls\n\nbody') },
    ],
  });
