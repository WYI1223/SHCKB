...
> **归档状态（2026-06-12 注记，mvp7 review）**：搬运已完成——本 repo 就是当年的"新 repo"。本目录现在是**历史归档**：grid-engine 等包已以演进后的形态住进 `packages/`，下文的结构提案/ADR 候选/决策 driver 均已被实际建设取代（现行权威 = `docs/` PRD/ADR 体系）。保留仅供考古；**任何内容不得作为现行依据引用**。可随时整体删除。
>
> （以下为搬运时的原文，未改动。）
>
